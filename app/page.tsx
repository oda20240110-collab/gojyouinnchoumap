...
// ---- ここは Next.js (Pages Router) 用のシンプル版です ----
// 貼り付け先: pages/index.tsx （TypeScriptでもOK）
// ※ 地図コンポーネントはサーバーでは動かないため、dynamic import（ssr:false）で読み込みます
...
// Leaflet のアイコンが表示されない問題対策（クライアント側でのみ設定）
function useLeafletIconFix() {
  useEffect(() => {
    if (typeof window === "undefined") return;
    (async () => {
      const L = await import("leaflet");
      const DefaultIcon = L.icon.default ? L.icon.default : L.icon;
    ...
    <div style={{minHeight:"100vh", background:"linear-gradient(#fff,#f8fafc)"}}>
      <div style={{maxWidth:1200, margin:"0 auto", padding:"16px"}}>
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", gap:12}}>
          <div>
            <h1 style={{fontSize:28, fontWeight:700}}>{t(lang,'title')}</h1>
            <p style={{color:"#475569"}}>{t(lang,'subtitle')}</p>
          </div>
          <div>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr", gap:12, marginTop:12}}>
          <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:8, alignItems:"center"}}>
            ...
            <div style={{display:"flex", gap:8, alignItems:"center"}}>
              <label style={{fontSize:12, opacity:.7}}>{t(lang,'addFromCSV')}</label>
              <input type="file" accept=".csv" onChange={(e)=> e.target.files && onCSV(e.target.files[0])} />
            </div>
          </div>

          <div style={{display:"flex", gap:12, alignItems:"center"}}>
            <span style={{fontSize:12, opacity:.7}}>{t(lang,'qr')}</span>
            <QRCodeCanvas value={typeof window!=="undefined"? window.location.href : ""} size={72} includeMargin />
            <button onClick={()=> navigator.clipboard.writeText(window.location.href)}>{t(lang,'openHere')}</button>
          </div>
            ...
            <MapContainer center={center} zoom={5} scrollWheelZoom style={{height:"100%", width:"100%"}}>
              ...
              {filtered.map((c:any) => (
                <Marker key={c.id} position={[c.lat, c.lng] as [number, number]}>
                  <Popup>
                    <div>
                      <div style={{fontWeight:700}}>{lang==='ja'?c.name_ja:lang==='en'?c.name_en:c.name_zh}</div>
                      <div style={{fontSize:12, color:"#15803d"}}>✅ {t(lang,'stampAvailable')}</div>
                      <div style={{fontSize:12, color:"#475569"}}>{c.prefecture} / {c.region}</div>
                      <div style={{display:"flex", gap:8, marginTop:4}}>
                        <a href={c.url} target="_blank" rel="noreferrer">Official</a>
                        <a href={c.gojoin_url} target="_blank" rel="noreferrer">{t(lang,'moreInfo')}</a>
                      </div>
                    </div>
                  </Popup>
                </Marker>
          ...
          <div>
            <h3 style={{fontWeight:600}}>{t(lang,'data')}</h3>
...
// ---- 追記が必要なファイル（1行だけ） ----
// pages/_app.tsx に次の1行を追加してください：
//   import 'leaflet/dist/leaflet.css'
// これで地図の見た目（タイル/ピン）が正しく表示されます。
